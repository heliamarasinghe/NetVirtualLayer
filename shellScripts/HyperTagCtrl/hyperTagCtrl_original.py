...
class LearningSwitch (object):
 ...
 # Tagging switch operation -------------------------------------------------
    ...
    # default l2_learning switch operation -----------------------------------
...
# draft switch operation
       # if event.port == trunk
           # if ip_packet is none 
               # if dl_dst == ff:ff:ff:ff:ff:ff <-- ARP packet
		   # dl_src_array = get all mac addresses for vlan from file
		   # for each element in dl_src_array
		       # if port available in macToPort table
		           # strip vlan tag and forward to port
		       # else <-- no entry in macToPort table
		           # send arp to find port
		           # if port found 
		               # update macToPort table
		               # strip vlan tag and forward to port
		           # else <-- arp timeout
		               # drop()

               # else <-- non-ip and non ARP packet
                   # log.error('un-treated packet received. Non-Ip and non-ARP')
                   # drop()

           # else <-- ip packet coming from trunk
               # if tag == 65535 <-- no vlan tag
                   # log.error('un-tagged packet arrived from trunk')
                   # drop()
               # else <-- ip packet coming from trunk with tag
		   # get vlan id from file for dl_dst
		   # if event.dl_vlan not equal to file.dl_vlan
		       # log.error('vlan mismatch detected. incoming vlan != vlan in file')
		       # drop
		   # else <-- incoming tag verified
		       # if dl_dst in macToPort table
		           # get corresponding port
		           # strip vlan tag and forward to port
                       # else <-- no entry in macToPort table for the ip packet coming from trunk with tag
		           # send arp to find port (flood)
		               # if port found 
		                   # update macToPort table
		                   # strip vlan tag and forward to port
		               # else <-- ARP timeout. VM unavailable
                                   # log.error('ARP time-out. dl_dst not found')
		                   # drop

       # else <-- any frame coming from local VM (access port)    
           # if dl_vlan == 65535
               # if dl_src address in file
                   # if dl_src not in macToPort table
                       #  add to macToPort table
	           # add tag and forward to trunk
	       # else <-- no dl_src address in file
                   # log.debug('unrecognized frame received. Set VLAN tag 100')
                   # add VLAN tag 100 and forward to Trunk
           # else <-- frame coming from access port is tagged
               # log.error('tagged packet came from local VM')
               # drop()
